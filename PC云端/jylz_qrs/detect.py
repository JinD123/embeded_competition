...
def run(
    ...
    source = str(source)#先将数据源（待检测）转换成str类型
    #判断是否保存图片
    save_img = not nosave and not source.endswith('.txt')  # save inference images
    is_file = Path(source).suffix[1:] in (IMG_FORMATS + VID_FORMATS)#判断是文件吗
    is_url = source.lower().startswith(('rtsp://', 'rtmp://', 'http://', 'https://'))#判断是网址吗
    webcam = source.isnumeric() or source.endswith('.streams') or (is_url and not is_file)#判断是不是数据流（例如数字时对应的摄像头或者以.stream结尾，或者说是网址不是文件）
...
def parse_opt(source):
    ...
    parser.add_argument('--source', type=str, default= "D:\\jylz_qrs\\data\\result", help='file/dir/URL/glob/screen/0(webcam)')
