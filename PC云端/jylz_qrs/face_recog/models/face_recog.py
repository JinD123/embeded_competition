...
predictor_68_point_model = "weights/shape_predictor_68_face_landmarks.dat"#pose_predictor_68_point是一个dlib库中已经训练好的人脸关键点检测器，
# 可以使用68个关键点来描述面部特征
...
def _raw_face_landmarks(face_image, face_locations):
    pose_predictor = pose_predictor_68_point#检测出68个特征点
    face_locations = [_css_to_rect(face_location) for face_location in face_locations]#将人脸信息转换成dlib库的矩阵对象

    return [pose_predictor(face_image, face_location) for face_location in face_locations]#利用pose_predictor对每张人脸返回一个具有68个关键点坐标的列表，表示该张人脸的特征
...
#compute_face_descriptor将人脸关键点坐标转换为一个128维的特征向量，表示该张人脸的面部特征

def face_distance(x1, x2, metric="norm"):#norm代表欧式距离
    """
