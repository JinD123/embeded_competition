...
class DynamicAttendanceApp(QWidget, Ui_DynamicAttendance):
    ...
    def open_source(self, source):
        ...
        def open_source_func(self):
            ...
            #将lambda数作为参数传递，lambda d: self.push_frame_signal.emit(d)可以看作是一个信号槽函数，用于向其他模块发送帧数据。
    ...
    def update_attended_students_num_lbl(self):
        ...
        print(attended_student_num)
        self.student_num_lbl.setText(f'人数:{attended_student_num}/{all_student_num}')
        for row in range(attended_student_num):
            item = self.attended_list.item(row)
            if item is not None:
                item_text = item.name
                print(item.text())
                attended_list_text.append(item_text)
        for row in range(self.absented_list.count()):
            item = self.absented_list.item(row)
            if item is not None:
                item_text = item.name
                print(item.text())
                absent_list_text.append(item_text)
        print("absent_list_text:",absent_list_text)
        merged_text = "a"+" ".join(attended_list_text)+"b"+" ".join(absent_list_text)
        print(merged_text)
        merged_text = merged_text.encode("utf-8")


        print("Merged text:", merged_text)
        client_socket2, client_address2 = self.server_socket2.accept()

        client_socket2.send(merged_text)
        print("发送")
