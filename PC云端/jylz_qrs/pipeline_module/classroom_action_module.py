...
peep_threshold = -70  # 第一个视频使用阈值-50
...
class CheatingActionModule(BaseModule):
    ...
    def __init__(self, weights, device='cpu', img_size=(480, 640), skippable=True):
        ...
        self.classifier = ClassroomActionClassifier(weights, device)#加载教室检测权重文件
    ...
    def process_data(self, data):
        ...
        #     # 头部姿态辅助判断转头
        #     # print(data.head_pose_euler)
        #
        #     for i in range(len(data.best_preds)):
        #
        #
        #
        #         if data.best_preds[i] == 1:
        #             if is_passing_list[i] != 0:
        #                 data.best_preds[i] = 1
        #     if data.head_pose_euler[i][1][0] < -45 or (
        #             data.head_pose_euler[i][0][0] < -25 and data.head_pose_euler[i][1][0] > -10 and
        #             data.head_pose_euler[i][1][0] < 20 and (
        #                     (data.head_pose_euler[i][2][0] < -5 and data.head_pose_euler[i][2][0] > -60)) or (
        #                     data.head_pose_euler[i][2][0] > 150 and data.head_pose_euler[i][2][0] < 160)) or (
        #             data.head_pose_euler[i][0][0] > 10 and data.head_pose_euler[i][1][0] < 0 and
        #             data.head_pose_euler[i][1][0] > -15 and data.head_pose_euler[i][2][0] > -175 and data.head_pose_euler[i][2][0] <0):
        #         data.best_preds[i] = 2
        #     # print(data.best_preds)
