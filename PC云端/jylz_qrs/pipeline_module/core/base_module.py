...
class TaskData:
    def __init__(self, task_stage, task_flag=TASK_DATA_OK):
        self.data = DictData()#所有的数据都保存在这个字典里
...
class TaskStage:
    ...
    def to_next_stage(self, task_data: TaskData):#在to_next_stage中将数据传递给下一个模块，
...
class BaseModule(ABC):
    ...
    def _run(self):
        ...
        while self.running:
            #在该基类中定义了他的运行方式
            task_data = self.product_task_data()#运行
            # 执行条件
    ...
    def product_task_data(self):
        try:
            task_data = self.queue.get(block=True, timeout=1)#任务数据队列
            self._refresh_process_interval_scale()
            return task_data
        except Empty:
