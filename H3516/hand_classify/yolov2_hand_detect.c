...
#define MODEL_FILE_HAND    "/userdata/models/hand_classify/face_detect_inst.wk" // darknet framework wk model
...
static HI_S32 HandDetect(uintptr_t model, IVE_IMAGE_S *srcYuv, DetectObjInfo boxs[])
{
    ...
    if (ret < 0) {
        SAMPLE_PRT("face detect Yolo2CalImg FAIL, for cal FAIL, ret:%d\n", ret);
        return ret;
    }
