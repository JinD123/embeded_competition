...
static pthread_t g_dataProcessThread = 0;
...
int times=0;
int uart_send(char *buffer)
{
  
char uartReadBuff[2048] ; /* 读取buff大小为2048 */
    unsigned int ret = 0;
   int  RecvLen=1024;
  ...
  uartFd2 = UartOpenInit();
   ...
   switch (send_or_read){
    case 0:
        {
        ret = UartSend(uartFd2,  buffer, strlen(buffer));//先发送信息
	    if(ret>0){//如果发送成功，即大于零
    ...
    case 1:
    {    
        ret=UartRead(uartFd2, uartReadBuff, RecvLen,100);
        if(strcmp(uartReadBuff,"ok")!=0&&times<=5)
        {
        send_or_read=1;
        times=times+1;
        printf("times:",times);
        printf("is reading\n");
            ...
            times=0;
         send_or_read=0;   
        memset(uartReadBuff, 0, strlen(uartReadBuff));
        }
        break;
    }
    default:break;
...
static HI_VOID HandClassifyAiProcess(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn)
{
    ...
    if (GetCfgBool("hand_classify_switch:support_hand_classify", true)) {
        if (g_workPlug.model == 0) {//加载手势识别模型
...
if (client_socket2 == -1) {
    ...
    if(connect(client_socket2, (struct sockaddr*)&server_addr, sizeof(server_addr)) == -1) {
        ...
        char buffer[buffer_size];
            ...
            ssize_t bytes_received = recv(client_socket2, buffer, buffer_size, 0);
...
static HI_VOID*  receiveData() {//线程函数
...
static HI_S32 HandClassifyAiThreadProcess(HI_VOID)
{
    ...
    s32Ret= pthread_create(&g_dataProcessThread, NULL, receiveData, NULL);
    return s32Ret;
}
...
HI_S32 SAMPLE_MEDIA_HAND_CLASSIFY(HI_VOID)
{
     ...
     */
    pthread_join(g_aiProcessThread, NULL);
    pthread_join(g_dataProcessThread, NULL);
